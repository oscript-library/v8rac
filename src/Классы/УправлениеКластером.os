...
//Создает администратора кластера
// 
//Параметры:
//	ПараметрыСозданияАдминистратораКластера - Соответствие
//	name
//	auth ["pwd,os"]
//	os-user
//	descr
...
// 
Функция СоздатьАдминистратораКластера(ПараметрыСозданияАдминистратораКластера) Экспорт
	
	ОбновитьИндексИнформационныхБаз(СписокЛокальныхКластеров()[0]);

	Параметры = СтандартныеПараметрыЗапуска();
		
	Параметры.Добавить("cluster");
	Параметры.Добавить("admin");	
	Параметры.Добавить("--cluster=" + СписокЛокальныхКластеров()[0]);
	Параметры.Добавить("register ");
	
	Для каждого ЭлементСтруктуры Из ПараметрыСозданияАдминистратораКластера Цикл
		
		Параметры.Добавить("--"+ ЭлементСтруктуры.Ключ + "=" + ЭлементСтруктуры.Значение);
		
	КонецЦикла;
	
	Возврат ВыполнитьКоманду(Параметры)
